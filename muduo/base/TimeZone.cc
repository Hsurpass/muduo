...
namespace muduo
{
  namespace detail
  {
    ...
    struct Localtime
    {
      ...
      bool isDst;       // 夏令时标记
      int arrbIdx;      // Data.names对应的时区名索引
    ...
    // 将秒数seconds转化为struct tm格式时间，参数utc来存储转换后的结果
...
/*
时区管理类，主要用于管理GMT时间和本地时间的转化
协调世界时UTC不与任何地区位置相关，也不代表此刻某地的时间，所以在说明某地时间时要加上时区
也就是说GMT并不等于UTC,而是等于UTC+0，只是格林威治时间刚好在0时区上
*/
...
namespace muduo
{
  namespace detail
  {
    // 提供文件读取工具函数，方便读取指定字节的文本信息，主要读取特定格式的时区文本信息
...
// 将1970/1/1 0:0:0 到此时的秒数seconds转化为struct tm格式
